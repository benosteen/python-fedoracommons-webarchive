...
class FedoraClient(object):
    ...
    def getDatastream(self, pid, dsid, params=None, date=None):
        ...
        if date:
            return self.fedora.request_get("objects/%s/datastreams/%s/%s" % (pid, dsid, date) , args=params)
        
        return self.fedora.request_get("objects/%s/datastreams/%s" % (pid, dsid), args=params)
        
    def getDatastreamUrl(self, pid, dsid, date=None):
        if self.use_UUID:
            pid = self.resolve_Non_UUID_pid(pid) or pid
        if date:
            return "%s/objects/%s/datastreams/%s/%s" % (self.server, pid, dsid, date)
        return "%s/objects/%s/datastreams/%s" % (self.server, pid, dsid)
    ...
    def listDatastreams(self, pid, format='xml', params=None, date=None):
        if self.use_UUID:
            pid = self.resolve_Non_UUID_pid(pid) or pid
        ...
        if format=='python':
            ...
            for dsid in dsid_list:
                ...
                if not ext:
                    datastream['winname'] = namespace + "_" + id_number+'-'+datastream['dsid'] + mimetypes.guess_extension(datastream['mimetype'])
                else:
                    if datastream['dsid'].endswith(ext):
                        datastream['winname'] = namespace + "_"+id_number+'-'+datastream['dsid']
                    else:
                        datastream['winname'] = namespace + "_"+id_number+'-'+datastream['dsid'] + '.' + ext
            ...
            if date:
                arg_list['asOfDateTime'] = date
                return self.fedora.request_get("objects/%s/datastreams" % (pid) , args=arg_list)
