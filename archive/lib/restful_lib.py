...
class Connection:
    ...
    def request_get(self, resource, args = None, headers={}, get_full_response=False):
        return self.request(resource, "get", args, headers=headers, get_full_response=get_full_response)
        
    def request_delete(self, resource, args = None, headers={}, get_full_response=False):
        return self.request(resource, "delete", args, headers=headers, get_full_response=get_full_response)
        
    def request_post(self, resource, args = None, body = None, filename=None, headers={}, get_full_response=False):
        return self.request(resource, "post", args , body = body, filename=filename, headers=headers, get_full_response=get_full_response)
    ...
    def request(self, resource, method = "get", args = None, body = None, filename=None, headers={}, get_full_response=False):
        params = None
        path = resource
        ...
        if self.query != "/":
            if self.query.endswith('/'):
                request_path.append(self.query[:-1])
            else:
                request_path.append(self.query)
            if path.startswith('/'):
                request_path.append(path[1:])
            else:
                request_path.append(path)
        
        conn.request(method.upper(), u'/'.join(request_path), body, headers)
